...
def process_video(vidcap, tile_dim):
    ...
    while success and cursor < frame_count:
        ...
        color_mean = calc_color_mean(image)
...
def process_target(target_image, split_level):
    ...
    for i in range(split_level):
        for j in range(split_level):
            ...
            tile = target_image[s]
            color_mean = calc_color_mean(tile)
...
def find_best_fit(frames, tile_color_means):
...
def validate_args(video_path, target_image_path):
    print(video_path)
...
def create_mozaik(video_path, target_image_path, recursion_level):

    validate_args(video_path, target_image_path)
    ...
    frames = process_video(vidcap, tile_dim)

    # list containing the color mean for each tile
    tile_color_means = process_target(target_image, split_level)

    indeces = find_best_fit(frames, tile_color_means)
...
parser.add_argument(
    "target_image", type=str, help="the mozaic will be made from this image"
)
parser.add_argument(
    "video",
    type=str,
    help="video where the frames will be used to create the mozaic tiles",
)
...
create_mozaik(video, target_image, recursion_level)
